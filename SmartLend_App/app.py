...
st.set_page_config(
    page_title="SmartLend Analytics | Modular Risk Dashboard",
    page_icon="💳",
    layout="wide",
...
def load_historical_sample_data():
    ...
    We sample only 20,000 rows to keep memory usage low.
    """
    # Look in the parent project data folder or local data folder
...
def get_shap_explainer(_model):
    ...
    Using leading underscore (_model) tells Streamlit NOT to hash the model object.
    """
...
if assets is None:
    ...
    with tab1:
        ...
        with st.form("loan_form"):
            ...
            with col1:
                ...
                # Scale amount up to ₹35 Lakhs ($42K USD)
            ...
            with col2:
                ...
                # Annual salary in Indian Rupees
        ...
        if submit_button:
            # Step A: Divide currency inputs by 83.0 to match the USD-trained model
            ...
            # Step B: Insert converted inputs
            ...
            # Ratios remain unit-free (since both numerator/denominator are scaled down)
            ...
            for col, val in categorical_inputs.items():
                if col in cat_mappings:
                    ...
                    val_str = str(val)
                    # Use frequency mapped value or default mode
                    input_row[col] = freq_map[val_str] if val_str in freq_map else freq_map[default_mode]
            
            # Convert to aligned DataFrame
            input_df = pd.DataFrame([input_row])
            input_df = input_df[features] # enforce features order
            
            # Step E: Make default risk prediction
            pd_prob = float(model.predict_proba(input_df)[0, 1])
            
            # Step F: Calculate Business Metrics
            lgd = 0.50 # 50% Loss Given Default standard
            ead_inr = loan_amnt_inr # Exposure is loan amount in INR
            ...
            st.write("This chart show exactly how each input feature contributed to increasing (red) or decreasing (green) risk.")
    ...
    with tab2:
        ...
        if hist_df is None:
            ...
            # Map grade floats to original strings
            ...
            max_inc_inr_display = 270000.0 * CONVERSION_RATE # ₹2.25 Crores limit
